...
## 问题现象
Ceph集群在进行读写性能测试后重启OSD节点时失败，测试工具报错“Slave hd2-0 prematurely terminated”，同时Ceph集群状态显示部分OSD处于down状态。

## 问题根因
“osd_memory_target”的值不是官方默认的4GB（4294967296字节），导致OSD进程在获取内存时失败。

## 解决方案
1. 通过命令“ceph --admin-daemon /var/run/ceph/ceph-osd.0.asoc config show | grep memory”确认osd_memory_target配置异常；
